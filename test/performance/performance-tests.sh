...
function update_benchmark() {
  ...
  kubectl create configmap config-mako -n "${TEST_NAMESPACE}" --from-file="${benchmark_path}/prod.config" \
    || abort "failed to create config-mako configmap"
  kubectl patch configmap config-mako -n "${TEST_NAMESPACE}" -p '{"data":{"env":"prod"}}' \
    || abort "failed to patch config-mako configmap"
